...
export default function TimetablePage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await listClasses({ pageSize: 200 });
    ...
    <div className="mx-auto max-w-6xl px-4 py-6">
      <div className="mb-5">
        ...
        <p className="mt-0.5 text-sm text-[var(--color-text-muted)]">
          View the weekly timetable for your classes. Timetables are managed by administrators.
        </p>
      </div>
      <TimetableWorkspace
        classes={classes}
        ...
        canManage={false}
        emptyMessage="You have no classes yet. Create a class first."
      />
    </div>
  );
}
