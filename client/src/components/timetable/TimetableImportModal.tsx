...
import { getImportTemplateUrl } from '@/services/timetable.api';
    ...
    >
      <div className="space-y-4">
        ...
        <label className="block cursor-pointer rounded-2xl border-2 border-dashed border-[var(--color-border)] bg-[var(--color-bg-subtle)] p-8 text-center transition-colors hover:border-[var(--color-primary)] hover:bg-[var(--color-primary-soft)]/50">
          ...
          <p className="mt-1 text-xs text-[var(--color-text-muted)]">Supports .xlsx, .xls</p>
          <input
            type="file"
            accept=".xlsx,.xls"
            className="hidden"
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button variant="primary" isLoading={importing} disabled={!file} onClick={onSubmit}>
            Import
          </Button>
        </div>
      </div>
